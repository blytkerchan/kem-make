...
Five KEM-MAKE protocol PDUs, wrapped in a versioned envelope:
...
`m` is `OCTET STRING OPTIONAL` (tagged `[0]`) on both
`SessionCompletionRequest` and `SessionCompletionResponse` — present only when the false-start
...
- **KEM public keys and ciphertexts carry an explicit `AlgorithmIdentifier`**
  with `parameters` absent (not `NULL`) per RFC 9935 and are length-checked against
  FIPS 203 on both the build and load paths — so a truncated or padded
  frame is rejected regardless of whether the bad data originated locally
  or over the wire.
...
`test_kem_make.py::test_schema_matches_python_classes_for_ack` cross-compiles
`kem-make.asn1` with `asn1tools` and confirms it produces byte-identical DER
to the `Ack` class, in both directions, for the one type that doesn't
require `ANY DEFINED BY` open-type registration to encode cleanly through
asn1tools. `AlgorithmIdentifier`'s open-type field is standard PKIX style
and compiles fine, but fully exercising it through asn1tools' encoder would
need that registration set up separately — it hasn't been, since it wasn't
needed to validate what changed so far.
...
you bump the pin, re-run `test_sit_make.py` (particularly
